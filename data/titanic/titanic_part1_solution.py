...
def display_passengers_per_gender():
    ...
    for record in records:
        gender = record[4]
        if gender.lower() == "male":              # makes sure that entry is converted to lower case
            males += 1
        else:
            females += 1
...
def display_passengers_per_age_group():
    children = adults = elderly = 0                           # All variables are equal to 0 (zero)

    for record in records:
        if record[5] != "":                                   # If record is NOT equal to an empty string
            age = float(record[5])
            if age < 18:
                children += 1
            elif age < 65:
